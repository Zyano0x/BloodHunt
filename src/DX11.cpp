...
void DX11Window::Hook()
{
	VIRTUALIZER_TIGER_LONDON_START;
	PresentFn = Signature("48 89 5C 24 ? 48 89 6C 24 ? 48 89 74 24 ? 57 41 56 41 57 48 83 EC ? 41 8B E8").Import("GameOverlayRenderer64.dll").GetPointer();
	ResizeBuffersFn = Signature("48 89 5C 24 ? 48 89 6C 24 ? 48 89 74 24 ? 57 41 56 41 57 48 83 EC ? 44 8B FA").Import("GameOverlayRenderer64.dll").GetPointer();
...
void DX11Window::Overlay(IDXGISwapChain* SwapChain)
{
	...
	if (!InitImGui) {
		if (SUCCEEDED(
			...
			io.Fonts->AddFontFromMemoryTTF(RudaFont, sizeof(RudaFont), 16.0f, nullptr,
